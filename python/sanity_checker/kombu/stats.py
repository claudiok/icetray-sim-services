...
cmlimit_t2 = n.asarray( [ .02480, .02878, .03177, .03430, .03656,
...
cmlimit_cumprob = n.hstack( (n.arange(.01,1,.01), [0.999]))
...
def cm_pvalue(t2):
    if t2 < cmlimit_t2[0]:
        ...
        idx = n.searchsorted(cmlimit_t2, t2)
...
def cumdist(where, data, weights):
    ...
    result = n.zeros(len(where), dtype=float)
    ...
    for i,x in enumerate(where):
        if x < data[0]:
            ...
            result[i] = wcum[n.searchsorted(data, x)]
...
def cumdist2(where, data,weights=None):
    ...
    wcum = None
    if weights is None:
        wcum = n.arange(1,len(data)+1,dtype=float) / len(data)
    else:
        weights = weights[idx]
        wcum = weights.cumsum() / weights.sum()
...
def cumdist3(where,data,weights):
    ...
    return n.asarray( [weights[data<=i].sum()  for i in where] ) / weights.sum()
...
def joint_cumdist(data1, data2, weights1=None, weights2=None):
    ...
    z = n.hstack((x,y))
    zidx = n.arange(1,len(x)+len(y)+1)
    sort_idx = n.argsort(z)
    z = z[sort_idx]
    m = sort_idx < len(x)
    r = zidx[m]
    s = zidx[-m]
    
    i = n.arange(1,len(x)+1)
    j = n.arange(1,len(y)+1)


    #FIXME
    Fslow = n.asarray( [weights1[idx][x<=tmp].sum()  for tmp in x] ) / weights1.sum()
    Gslow = n.asarray( [weights2[idx][y<=tmp].sum()  for tmp in y] ) / weights2.sum()
...
def cmstat_1(data1, data2):
    ...
    x = n.asarray(data1, dtype=float)
    y = n.asarray(data2, dtype=float)
    x.sort()
    y.sort()
    lx = float(len(x))
    ly = float(len(y))
    ...
    F = n.asarray( [len(x[x<=i]) / lx for i in z] )
    G = n.asarray( [len(y[y<=i]) / ly for i in z] )

    L =float(lcm(len(x), len(y)))
    h2 = L * (F - G)
    T2 = lx*ly / (lx+ly)**2 * n.power(F - G, 2).sum()
    return T2
...
def cmstat_1w(data1, data2, weights1, weights2):
    ...
    # FIXME: inefficient
    F = n.asarray( [w1[x<=i].sum()  for i in z] ) / w1.sum()
    G = n.asarray( [w2[y<=i].sum()  for i in z] ) / w2.sum()
...
def cmstat_2(data1,data2):
    ...
    y.sort()
    zidx = n.arange(1,len(x)+len(y)+1)
    sort_idx = n.argsort(n.hstack((x,y)))
    m = sort_idx < len(x)
    ...
    # testing only
    #F = n.vectorize(lambda i : len(x[x<=i]) / float(len(x)))
    #G = n.vectorize(lambda i : len(y[y<=i]) / float(len(y)))
    #assert ( (i/float(len(x)) - (r-i)/float(len(y))) == (F(x)-G(x)) ).all()
    #assert ( ((s-j)/float(len(x)) - (j)/float(len(y))) == (F(y)-G(y)) ).all()

    U = len(x) * n.power((r-i),2).sum() + len(y)*n.power((s-j),2).sum()
...
def cmstat_3(data1, data2, weights1, weights2):
    ...
    T2 = len(x)*len(y) / float(len(x)+len(y))**2 * (n.power(FGx, 2).sum() + n.power(FGy,2).sum())
    return T2
...
def cmstat_2sample(x,y,w1=None,w2=None):
    ...
    Hsteps = n.hstack((H[0],H[1:]-H[:-1]))
    T = (w1sum*w2sum) / (w1sum + w2sum) * (n.power(F-G, 2) * Hsteps).sum()
    return T
...
class freq_func(defaultdict):
    ...
    def values(self):
        ...
        zeta = n.asarray([i for i in self.keys() if self[i] != 0], dtype=float)
        zeta.sort()
        return zeta
    ...
    def frequencies(self):
        ...
        return n.asarray([self[i] for i in self.values])
