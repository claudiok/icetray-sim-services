...
for e,p in dom_geo:
    ...
    if e not in badOMs and e in dom_cal and e in dom_status:
        ...
        ## For IceTop transit times are different (because of different gains, see Chris Weaver's presentation in IceCal Call 06-10-2011)
            # (26,61) is known to be a bit different sometimes
        elif ((not (transitTime > 130 and transitTime < 165)) and (e.om >= 61 and e.om <= 64)) or (e.om == 61 and e.string == 26): 
            print 'IceTop : %s  transitTime = %s  voltage = %s (behaviour should be ~2000/sqrt(voltage)+87.21) !!' \
                  % (str(e), transitTime,status_this_om.pmt_hv/icetray.I3Units.volt)
       ###########
        ...
        if e.om> 60:
            ...
            if( (status_this_om.dom_gain_type==dataclasses.I3DOMStatus.DOMGain.High) \
                and (vemcal_this_om.hglg_cross_over <= 1500) \
                or (vemcal_this_om.hglg_cross_over > 4000)): # weird pybinding name corrected in the dataclasses trunk
